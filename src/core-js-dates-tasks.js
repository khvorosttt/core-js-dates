...
function getTime(date) {
  ...
  return `${numberFormat(hours)}:${numberFormat(minutes)}:${numberFormat(seconds)}`;
}
...
function getNextFriday(date) {
  let day = 5 - date.getDay();
  if (day <= 0) {
    day = 7 + day;
  }
  const fridayDate = new Date(date);
  fridayDate.setDate(date.getDate() + day);
  return fridayDate;
}
...
function formatDate(date) {
  ...
  return `${dateObj.getUTCMonth() + 1}/${dateObj.getUTCDate()}/${dateObj.getUTCFullYear()}, ${hours}:${numberFormat(dateObj.getMinutes())}:${numberFormat(dateObj.getSeconds())} ${dateObj.getUTCHours() < 12 ? 'AM' : 'PM'}`;
}
...
function getWeekNumberByDate(date) {
  ...
  dateStart.setFullYear(date.getFullYear(), 0, 1);
  let days = Math.ceil(
    (date.getTime() - dateStart.getTime()) / (1000 * 60 * 60 * 24) + 1
  );
  let weeks = dateStart.getDay() === 1 ? 0 : 1;
...
function dateFormat(date) {
  return `${numberFormat(date.getDate())}-${numberFormat(date.getMonth() + 1)}-${date.getFullYear()}`;
}

function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startInfo = period.start.split('-');
  const endInfo = period.end.split('-');
  const start = new Date();
  start.setFullYear(startInfo[2], startInfo[1] - 1, startInfo[0]);
  const end = new Date();
  end.setFullYear(endInfo[2], endInfo[1] - 1, endInfo[0]);
  const schedule = [];
